...
mongoose.connection.on("error", err => {
    log.error("MongoDB failed to connect, make sure MongoDB is opened in the background.");
    throw err;
});
...
fs.readdirSync("./Api").forEach(fileName => {
    try {
        ...
        log.error(`OGFN API Error: Failed to load ${fileName}`)
    }
});
...
if (config.bEnableHTTPS) {
    server = httpsServer.listen(PORT, () => {
        log.backend(`Backend started on the port: ${PORT} (SSL Enabled)`);
        ...
        if (err.code === "EADDRINUSE") {
            log.error(`The port ${PORT} is already in use!\nClosing in 3 seconds...`);
    ...
    server = app.listen(PORT, () => {
        log.backend(`Backend started on the port: ${PORT} (SSL Disabled)`);
        require("./xmpp/xmpp.js");
        if (config.discord.bUseDiscordBot === true) {
            require("./DiscordBot");
        }
...
if (config.Website.bUseWebsite === true) {
    ...
    if (config.bEnableHTTPS) {
        ...
        websiteApp.listen(config.Website.websiteport, () => {
            ...
            if (err.code === "EADDRINUSE") {
                log.error(`Sito porta ${config.Website.websiteport} è già in uso!\nChiudendo in 3 secondi...`);
                await functions.sleep(3000);
                process.exit(1);
            } else {
                throw err;
